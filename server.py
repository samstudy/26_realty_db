...
def ads_list(page=1):
    post_per_page = 10
    ads = Flats.query.filter(Flats.is_expired==False).paginate(page,
                                                               post_per_page, False)
...
def search(page=1):
    ...
    session['new_building'] = request.args.get('new_building',
                                                True, type=bool)
...
def results(page=1):
    post_per_page = 10
    ads = Flats.query.filter(Flats.oblast_district == session['dist'],
                             Flats.is_expired == False, Flats.price > session['minv'],
                             Flats.price < session['maxv'],
